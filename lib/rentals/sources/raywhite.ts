...
const SEARCH_URL =
  "https://rwblenheim.co.nz/properties/residential-for-rent?category=&suburbPostCode=";
...
function absoluteUrl(href: string): string | null {
  try {
    return new URL(href, "https://rwblenheim.co.nz").toString();
  } catch {
    return null;
  }
}
...
export function parseRayWhiteHtml(html: string): Rental[] {
  ...
  for (const match of html.matchAll(detailLink)) {
    ...
    const id = `raywhite:${sourceListingId}`;
    ...
    rentals.push({
      ...
      propertyManager: "Ray White Blenheim",
      contactName: "Ray White Blenheim Property Management",
      contactPhone: "+64 3 578 8059",
      contactEmail: "enquiries.blenheim@raywhite.com",
      ...
      followUpAction: "Open listing, book viewing or contact Ray White Blenheim",
      rating: null,
    });
  }
...
async function fetchRayWhiteRentals(): Promise<Rental[]> {
  ...
  if (!response.ok) {
    throw new Error(`Ray White returned HTTP ${response.status}`);
  }

  const rentals = parseRayWhiteHtml(await response.text());
  if (rentals.length === 0) {
    throw new Error("Ray White returned no local residential rentals");
  }

  return rentals;
}

export const rayWhiteSource: RentalSourceAdapter = {
  name: "Ray White Blenheim",
  enabled: process.env.RAYWHITE_ENABLED !== "false",
  fetchRentals: fetchRayWhiteRentals,
};
